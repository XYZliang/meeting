...
Page({
  ...
  */
  data_click: function (e) {
    ...
    this.data.select.room = this.data.rooms.find(r => {
      return r.id == room_id
    })
    if (!this.data.select.click) {
    ...
    this.data.select.end_real = app.time.valueToTime(app.time.parseTime(this.data.select.end).value() + 10 * 60).string(2)
    this.setData({
      select: this.data.select
    })
    this.check_td_data()
  ...
  onLoad: function (options) {
    ...
    if (options.date) {
      this.selectComponent("#date_select").setData({
